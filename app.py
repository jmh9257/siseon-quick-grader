...
ANSWER_KEYS = {
    ...
    "2회": {
        "공통": {i: CHOICE_TO_MARK[str(((i + 1) % 5) + 1)] for i in range(1, 35)},
        "화법과작문": {i: CHOICE_TO_MARK[str(((i + 2) % 5) + 1)] for i in range(35, 46)},
        "언어와매체": {i: CHOICE_TO_MARK[str(((i + 3) % 5) + 1)] for i in range(35, 46)},
    },
    "3회": {
        "공통": {i: CHOICE_TO_MARK[str(((i + 2) % 5) + 1)] for i in range(1, 35)},
        "화법과작문": {i: CHOICE_TO_MARK[str(((i + 3) % 5) + 1)] for i in range(35, 46)},
        "언어와매체": {i: CHOICE_TO_MARK[str(((i + 4) % 5) + 1)] for i in range(35, 46)},
    },
}
...
POINTS = {i: 2 for i in range(1, 46)}
...
def grade_answers(exam_round, elective, answers):
    ...
    for q_no in range(1, 46):
        ...
        result[q_no] = {
            ...
            "배점": POINTS[q_no],
            "획득점수": POINTS[q_no] if is_correct else 0,
        }
...
st.title(APP_TITLE)
st.caption("구간별 답안을 숫자로 입력하면 자동 채점됩니다. ①②③④⑤는 각각 1, 2, 3, 4, 5로 입력하세요.")
